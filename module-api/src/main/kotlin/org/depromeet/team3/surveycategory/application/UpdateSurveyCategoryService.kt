    ...
    operator fun invoke(id: Long, request: UpdateSurveyCategoryRequest): Unit {
        ...
        val parentCategory = request.parentId?.let { parentId ->
            val parent = surveyCategoryRepository.findByIdAndIsDeletedFalse(parentId)
                ?: throw DpmException(ErrorCode.PARENT_CATEGORY_NOT_FOUND, mapOf("parentId" to parentId))
            
            // 부모 카테고리가 BRANCH 레벨인지 확인
            if (parent.level != org.depromeet.team3.survey_category.SurveyCategoryLevel.BRANCH) {
                throw DpmException(ErrorCode.INVALID_REQUEST, mapOf("reason" to "부모 카테고리는 BRANCH 레벨이어야 합니다", "parentLevel" to parent.level))
            }
            
            // 부모-자식 타입 호환성 검증
            if (parent.type != request.type) {
                throw DpmException(ErrorCode.INVALID_REQUEST, mapOf("reason" to "부모 카테고리와 자식 카테고리의 타입이 일치하지 않습니다", "parentType" to parent.type, "childType" to request.type))
            }
        ...
        if (existingCategory.level == org.depromeet.team3.survey_category.SurveyCategoryLevel.BRANCH && 
            ...
            if (childCount > 0) {
                throw DpmException(ErrorCode.INVALID_CATEGORY_LEVEL_CHANGE, mapOf("childCount" to childCount))
            }
        }
        ...
        if (surveyCategoryRepository.existsByNameAndParentIdAndIsDeletedFalse(request.name, request.parentId, id)) {
            throw DpmException(ErrorCode.DUPLICATE_CATEGORY_NAME, mapOf("name" to request.name, "parentId" to request.parentId))
        }
        ...
        if (surveyCategoryRepository.existsByOrderAndParentIdAndIsDeletedFalse(request.order, request.parentId, id)) {
            throw DpmException(ErrorCode.DUPLICATE_CATEGORY_ORDER, mapOf("order" to request.order, "parentId" to request.parentId))
        }
